...
int GetHiPriCildIDX(Heap* ph, int idx) {  // 두 개의 자식 노드 중 높은 우선순위의 자식 노드 인덱스 값 반환
    if (GetLChildIDX(idx) > ph->numOfData)  // 자식 노드가 존재하지 않는다면,
        ...
        if (ph->comp(ph->heapArr[GetLChildIDX(idx)], ph->heapArr[GetRChildIDX(idx)]) < 0)  // 오른쪽 자식 노드의 우선순의가 높다면,
...
void HInsert(Heap* ph, HData data) {  // 힙에 데이터 저장
    ...
    while (idx != 1) {
        ...
        if (ph->comp(data, ph->heapArr[GetParantIDX(idx)]) > 0) {  // 새 노드의 우선순위가 높다면
