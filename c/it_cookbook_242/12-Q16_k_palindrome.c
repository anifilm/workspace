/* 16
한글 문장을 입력받아 그것이 회문인지를 판단하는 프로그램을 작성하라. 단, 빈칸이나 기호(아스키코드 32에서 47까지)는
무시하기로 한다.
힌트 first를 0으로, last를 배열 끝으로 초기화한 다음 first와 last가 교차할 때까지 대칭 위치의 문자를 비교한다.
first는 2바이트씩 전진, last는 2바이트씩 후퇴, 빈칸이나 기호는 건너뛴다.
