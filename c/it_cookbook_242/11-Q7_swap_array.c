/* 07
main에서 int arr[5], arr2[5];라는 배열을 선언하고 print_array 함수를 불러 입력 내용을 출력하라. 이후 두 배열과
요소의 개수를 인자로 넘겨주면서 swap_array 함수를 호출하여 배열 내용을 서로 바꾸어 보라. 이후 main으로 되돌아와서
배열 내요이 바뀌었는느 다시 print_array 함수를 불러 출력하라. 단, 두 함수 모두 포인터 기호만 써서 구현해야 하며
배열 요소의 개수는 다섯 개로 한다.
