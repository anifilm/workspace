/* 07
배열에 원하는 작업을 가하는 데에도 함수 포인터를 사용할 수 있다. 아래 예에서 for_each_element 함수는 루프를 돌면서
배열의 각 요소에 대해 인자로 전달된 함수를 가하는 역할을 한다. 이런 종료의 함수를 map(mapping, 사상) 함수라 부른다.
어떤 함수가 전달되든 그 함수를 적용하기 때문에 map 함수는 범용성이 높다고 할 수 있다. 아래 프로그램을 완성하고 실행
해 보라.
*/
#include <stdio.h>
