// 14 일련의 정수를 입력받을 때 미리 몇 개가 들어오는지 알고 있으면 그 개수만큼 루프를 돌면서
// 읽으면 된다. 그러나 그렇지 않을 경우가 문제다. 입력 끝은 엔터키("\n")에 해당하는 문자형이
// 므로 정수처럼 계속해서 %d 형식으로 읽을 수가 없고 별도로 %c 형식으로 읽어야 하기 때문이다.
// 물론 -1을 입력 끝을 알리는 센티넬로 약속하야 -1을 읽으면 입력을 멈출 수도 있다. 그러나 -1
// 도 입력 데이터로 들어올 수 있으므로 이 역시 문제가 된다. 한 가지 대안은 모든 입력을 일단
// 문자(열) 읽어 배열에 저장하고 이후 배열을 읽으면서 atoi 함수를 써서 문자열을 정수로 변환
// 하는 것이다. 입력 정수를 문자(열) 형태로 str 배열에 저장한 후 그 배열을 다시 읽어 arr이라
// 는 정수 배열에 저장하는 프로그램을 완성해 보라. 단, 설명은 편의상 널 테스트는 생략하기로
// 한다.
