/* 10
회문인지 판단하는 프로그램을 작성하되 아래 재귀 함수를 호출하라. 여기서 first, last는 비교 해야 할 요소의
인덱스를 말한다.
힌트 첫 문자와 마지막 문자를 비교하여 같으면 ++first, --last에 의해 비교 범위를 줄여서 재귀 호출을 하면
된다. 베이스 케이스는 두 인덱스가 교차할 때, 즉 first가 last보다 크거나 같을 때다.
