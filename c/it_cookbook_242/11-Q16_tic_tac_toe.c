// 16 아래 프로그램은 7장의 tic-tac-toe 프로그램을 구현한 예다. 일단 소스코드를 이해한 후에
// computer_move 함수, check 함수, 기타 필요한 함수를 수정하되 세 가지 전략을 부여하라.
// 첫째, 랜덤 함수를 사용하여 위치를 결정하라. 둘째, 사람이 놓은 돌이 일직선이 되는 것을
// 예방 할 수 있는 위치에 놓아보라. 셋째, 컴퓨터 자신이 놓은 돌이 일직선이 될 수 있는 위치에
// 놓아보라. 더 나은 전략이 있으면 그 전략을 구현해서 컴퓨터가 사람을 이길 수 있는 방법을
// 모색해보라.
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
