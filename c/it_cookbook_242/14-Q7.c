/* 07
과제 6번에서는 단어 수에 해당하는 count 변수를 사전 구조체 정의에 포함시키는 바람에 사전 전체가 하나의 구조체로 되어서
파일에 저장할 때도 사전 전체를 다시 저장할 수 밖에 없었다. 그러나 만약 사전 구조체 정의에서 count 변수를 빼면 파일의
모든 내용을 동일하게 wordType으로 가져갈 수 있다. 따라서 프로그램을 빠져 나갈 때에도 이전 파일의 끝에 새로운 단어만
추가할 수 있다. 아래 프로그램을 완성하여 append 모드에서 파일을 저장할 수 있게 하라.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
