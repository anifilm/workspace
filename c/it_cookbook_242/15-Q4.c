/* 04
위 프로그램은 MAX를 10으로 규정하고 있기 때문에 여는 괄호가 열 개보다 많을 경우에 루프 내부에서 Stack full 메시지를
내보내고 프로그램이 끝나버린다. 이는 루프를 빠져나와 스택이 비었는지를 비교하기도 전의 일이다. 이 문제를 해결하려면
배열 버전 대신 포인터 버전을 써서 여는 괄호가 들어올 때마다 동적으로 새로운 노드를 만들어야 한다. 예제에서 작성한
포인터 버전의 스택을 사용하여 위 main 프로그램을 포인터 버전으로 수정하라.
