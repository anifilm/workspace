/* 10
현재 시간을 출력한 후 몇 년, 몇 개월의 시간을 되돌리고 싶은지를 입력받아 새로운 시간을 출력하라.
힌트 일단 현재 시간을 출력한다. 이후 time 함수의 리턴 값을 인자로 해서 localtime 함수를 호출하고 그 리턴 값으로
struct tm 타입의 시간 구조체를 채울 수 있다. 이 구조체의 tm_year와 mt_month 값을 조정한 후에 mktime 함수를 호출
하여 초 단위로 바꾸면 다시 ctime 함수를 써서 새로운 시간을 출력할 수 있다.
