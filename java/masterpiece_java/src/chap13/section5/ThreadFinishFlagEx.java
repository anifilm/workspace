...
class RandomThread extends Thread {
    ...
    public void run() {
        while (true) {
            ...
            label.setLocation(x, y); // 레이블의 콘텐츠 팬 내의 임의의 위치로 설정
            contentPane.add(label);  // 레이블을 콘텐츠 팬에 추가
            contentPane.repaint(); // 콘텐츠 팬을 다시 그려 추가된 레이블이 보이게 함
            try {
                Thread.sleep(300); // 0.3초 동안 잠을 잔다.
                if (flag == true) {
                    contentPane.removeAll(); // 콘텐츠 팬에 있는 모든 레이블 제거
                    label = new JLabel("finish");
                    label.setSize(80, 30); // "finish" 레이블의 크기 80x30
                    label.setLocation(100, 100); // 레이블을 콘텐츠 내의 임의의 위치로 설정
...
public class ThreadFinishFlagEx extends JFrame {
    ...
    public ThreadFinishFlagEx() {
        ...
        th = new RandomThread(c); // 스레드 생성, 스레드에 콘텐츠 팬 전달
        th.start(); // 스레드 시작
    }
