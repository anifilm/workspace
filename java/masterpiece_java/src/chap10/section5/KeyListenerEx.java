...
public class KeyListenerEx extends JFrame {
    ...
    public KeyListenerEx() {
        ...
        // 3개의 레이블 컴포넌트를 콘텐츠 팬에 부착
        for (int i = 0; i < keyMessage.length; i++) {
            c.add(keyMessage[i]);
            keyMessage[i].setOpaque(true); // 배경색이 보이도록 불투명 속성 설정
            keyMessage[i].setBackground(Color.YELLOW); // 배경색을 YELLOW로 설정
        }
    ...
    class MyKeyListener extends KeyAdapter {
        ...
        public void keyReleased(KeyEvent e) {
            System.out.println("KeyReleased"); // 콘솔창에 메서드 이름 출력
        }

        public void keyTyped(KeyEvent e) {
            System.out.println("KeyTyped"); // 콘솔창에 메서드 이름 출력
        }
    }
