...
def is_between_100_and_999(user_input_number):
    ...
    is_number = int(user_input_number)

    if is_number >= 100 and is_number < 1000:
        result = True
    else:
        result = False
...
def is_validated_number(user_input_number):
    ...
    if is_digit(user_input_number) is True:
        if is_between_100_and_999(user_input_number) is True:
            if is_duplicated_number(user_input_number) is not True:
                result = True
...
def get_not_duplicated_three_digit_number():
    ...
    while 1:
        if is_duplicated_number(random_number) is True:
...
def get_strikes_or_ball(user_input_number, random_number):
    ...
    u_num = set(user_input_number)
    r_num = set(random_number)

    num_union = u_num & r_num
    ball = len(num_union)

    if ball > 0:
        u_str = str(user_input_number)
        r_str = str(random_number)

        for i in range(3):
            if u_str[i] is r_str[i]:
                strike += 1
                ball -= 1
...
def is_yes(one_more_input):
    ...
    is_input = one_more_input.upper()

    if is_input == 'Y' or is_input == 'YES':
        result = True
    else:
        result = False
...
def main():
    ...
    # 위의 코드를 포함하여 자유로운 수정이 가능함
        while 1:
            ...
            print('Strikes :', get_strikes_or_ball(user_input, random_number)[0], ', Balls :', get_strikes_or_ball(user_input, random_number)[1])
        ...
        while (user_input is not 0):
            ...
            if is_yes(cont_input) is True:
                user_input = 999
                break
            elif is_no(cont_input) is True:
                user_input = 0
            else:
                print('Wrong Input, Input again')
