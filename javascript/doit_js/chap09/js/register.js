...
function checkId() {
  if (userId.value.length < 4 || userId.value.length > 15) { // userId 필드 내용의 길이가 4이하이거나 15이상일 경우 실행
    alert("4~5자리의 영문과 숫자를 사용하세요."); // 오류 메시지 출력
...
function checkPw() {
  if (pw1.value.length < 8) {
    alert("비밀번호는 8자리 이상이어야 합니다."); // 오류 메시지 출력
...
function comparePw() {
  if (pw1.value != pw2.value) {
    alert("암호가 다릅니다. 다시 입력하세요."); // 오류 메시지 출력
