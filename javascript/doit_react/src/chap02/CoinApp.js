...
class CoinApp {
  constructor(coins) {
    this.coins = Object.entries(coins).reduce(
      (coins, [code, amount]) => ({
        ...coins,
  ...
  buy(code, price, amount) {
    ...
    const { microAmount: currentAmount, price: currentPrice } = this.coins[code];
  ...
  sell(code, price, amount) {
    ...
    const { microAmount: currentAmount = 0, price: currentPrice } = this.coins[code] || {};
  ...
  print() {
    ...
    Object.entries(this.coins).forEach(([code, { microAmount, price }]) => console.log(`${code}|${price}|${(microAmount * price) / MICRO}`));
  }
}
